...
void NFunctionMesher::arrange_on_triangle(const std::vector<EVector2>& triangle,
    ...
    vector<bool> isPencilActive(linePencils.size());   //used to check if linepencil even intersects the triangle, so as to bother with computing intersections
    for (int i = 0; i < linePencils.size(); i++) {
        ...
        linepencil_triangle_intersection(linePencils[i], triangle, intEdges, intFaces, inParams, outParams, triParams);
        ...
        for (int j=0;j<linePencils[i].numLines;j++){
            if (!intEdges[j] && !intFaces[j])
                continue;   //no (non-measure-zero) intersection of that line with the triangle
            
            isPencilActive[i]=true;
            
            SegmentData newData; newData.isFunction=true;
    ...
    Matrix<ENumber, Eigen::Dynamic, 2> I2dts(2*linePencils.size()*linePencils.size(),2);   //It's double the information, but N is usually quite small so it's negligble and easier indexing.
    Matrix<ENumber, Eigen::Dynamic, 1> t00s(2*linePencils.size()*linePencils.size(),1);
    for (int i=0;i<linePencils.size();i++){
        if (!isPencilActive[i])
            continue;
        for (int j=i+1;j<linePencils.size();j++){
            ...
            EInt iso1Overlap;  //will not be used since line pencils of parametric lines are not supposed to overlap
            linepencil_intersection(linePencils[i], linePencils[j], t00, I2dt,iso1Overlap);
...
void NFunctionMesher::segment_arrangement(const std::vector<Segment2>& segments,
    ...
    for (int i=3;i<segments.size();i++) {   //starting from non-triangle segments
    ...
    for (int i=0;i<SV.size();i++){
        ...
        for (std::set<std::pair<ENumber, int>>::iterator si = SV[i].begin(); si!=SV[i].end();si++){
            ...
            if (nextsi!=SV[i].end()) {
                ...
                std::vector<SegmentData> newEdgeData(1); newEdgeData[0]=data[i];
    ...
    for (int i=0;i<arrVertices.size();i++) {
        ...
        for (int s=0;s<edgeOrder.size();s++) {
            ...
            int outCurrHE = (outgoing ? triDcel.edges[adjArrEdges[edgeOrder[s]].first].halfedge  : triDcel.halfedges[triDcel.edges[adjArrEdges[edgeOrder[s]].first].halfedge].twin);
            ...
            int outNextHE = (outgoing ? triDcel.edges[adjArrEdges[edgeOrder[nexts]].first].halfedge  : triDcel.halfedges[triDcel.edges[adjArrEdges[edgeOrder[nexts]].first].halfedge].twin);
    ...
    for (int i=0;i<triDcel.halfedges.size();i++) {
        ...
        do {
            ...
            counter++;
            assert (counter<10000 && "NFUnctionMeshes::generate_mesh(): something wrong with the generated faces");
    ...
    assert(triDcel.check_consistency(false, true, true, false) && "DCEL not consistent!");
    ...
    assert(outerFace!=-1 && "NFunctionMeshes::generate_mesh(): Didn't find outer face!");
...
void NFunctionMesher::generate_mesh(const unsigned long resolution = 1e7) {
    ...
    for (int findex = 0; findex < origMesh.F.rows(); findex++) {
        ...
        for (int funcIter = 0; funcIter < mData.N; funcIter++) {
            ...
            EVector2 gradVector = triExactNFunction[2 * mData.N + funcIter] * EVector2({-e01[1], e01[0]}) +
            triExactNFunction[0 * mData.N + funcIter] * EVector2({-e12[1], e12[0]}) +
            triExactNFunction[1 * mData.N + funcIter] * EVector2({-e20[1], e20[0]});
    ...
    assert(genDcel.check_consistency(true, false, false, false) && "NFunctionMeshes::generate_mesh(): DCEL not consistent!");
}
